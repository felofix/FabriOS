...
class FindMyFeeders:
	...
	def find_hotspots(self, n_hotspots):
		"""
		Very simply finding the hottest spots.
		just saying that values over 95 % of max are hotspots. 

		As of now you choose the number of hotspots, but this
		has to be automatic. 
		"""
	...
	def find_highest_point_above(self, given_point, dimension):
		"""
		Only works for z up direction. 

		"""
		# Extract the x and y coordinates of the given point
		gx, gy = given_point[0], given_point[1]
		
		# Filter points that have the same x and y coordinates as the given point
		same_xy_points = self.grid_points[(self.grid_points[:, 0] == gx) & (self.grid_points[:, 1] == gy)]
		
		# If no points have the same x and y coordinates, return None or handle as needed
		if same_xy_points.size == 0:
			return None

		# Find the point with the maximum z value among the filtered points
	...
	def generate_cylindrical_feeders(self, radius, height, num_points_surface=10):
		"""
		Generates a feeder based on hotspots. 
		"""
		self.feeder_points = []

		for h in self.highest_points:
			# Unpack the lowest point
			x0, y0, z0 = h
	...
	def assign_heat(self, point_idx):
		"""
		Assings some heat based on the heat value. 
		"""
	...
	def create_heat_map(self, resolution):
		"""
		Create some bogus heatmap so that I can 
		get the functionality going. 
		"""
